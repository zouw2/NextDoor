...
pvalue, selection_frequency, model_score){
    ...
    for(j in S){
        coefj = rep(0, p)
        coefj[-j] = coef(models[[j]], s = model0$lambda[index])[-1,1] #the first row is '(Intercept)' for other models, but for cox models, there seems no coefficient row from coef. 
